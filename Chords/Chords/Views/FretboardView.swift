...
// MARK: - Setup
    ...
    func setupCapo(for proxy: GeometryProxy) -> some View {
    ...
    func gridWidth(for proxy: GeometryProxy) -> CGFloat {
        return proxy.size.width / CGFloat(frets.count + 1)
    }
    
    func gridHeight(for proxy: GeometryProxy) -> CGFloat {
        return proxy.size.height / CGFloat(fretLineCount + 1)
    }
    
    func setupFrets(with proxy: GeometryProxy) -> some View {
    ...
    func setupStrings(with proxy: GeometryProxy) -> some View {
        HStack(spacing: gridWidth(for: proxy)) {
            ForEach(frets, id: \.self) { water in
                Group {
                    if water >= 0 {
                        Color.primary
                    } else {
    ...
    func setupStringOverlay(at index: Int, with proxy: GeometryProxy) -> some View {
        Group {
            if shouldShowFingers(for: index) {
                Color.primary
                    .clipShape(Circle())
    ...
    func shouldShowFingers(for index: Int) -> Bool {
        return fingers[index] > 0 && (barres.isEmpty || (!barres.isEmpty && frets[index] != barres.first!))
    }
    
    func calculateOffset(index: Int, proxy: GeometryProxy, isNumber: Bool = false) -> CGSize {
        let gridSize = CGSize(
            width: gridWidth(for: proxy),
            height: gridHeight(for: proxy)
        )
